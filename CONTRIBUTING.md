...
3. `$ yarn start` or `$ npm run start` to validate you've got it working
...
>
> ```
> git remote add upstream https://github.com/hadabo/express-poc.git
> git fetch upstream
...
`yarn test` or `npm run test` which will update any snapshots that need updating.
Make sure to include those changes (if they exist) in your commit.

### opt into git hooks

There are git hooks set up with this project that are automatically installed
when you install dependencies. They're really handy, it will lint the code and
give you warning messages by default (so as to not hinder new contributors).
